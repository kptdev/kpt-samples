...
TRANSLATIONS =
  begin
    ...
    YAML.load_file(File.join(__dir__, "locales", "en.yml")) || {}
  end

# Dotted-key lookup, e.g. t("email.subject"). Returns the key itself on miss.
def t(key)
  result = TRANSLATIONS
  key.split(".").each do |part|
    return key unless result.is_a?(Hash)
    result = result[part]
  end
  result.is_a?(String) ? result : key
end
