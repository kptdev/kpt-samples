...
# i18n
LOCALE_CODE = ENV.fetch("NEXT_PUBLIC_LOCALE", "en-US").split("-").first || "en"
LOCALE_FILE = File.exist?("locales/#{LOCALE_CODE}.yml") ? "locales/#{LOCALE_CODE}.yml" : "locales/en.yml"
TRANSLATIONS = YAML.load_file(LOCALE_FILE)

def t(key)
  key.split(".").reduce(TRANSLATIONS) { |hash, k| hash[k] }
end
